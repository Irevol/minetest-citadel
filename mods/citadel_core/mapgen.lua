...
local function sepia_hud_create(player)
	citadel.sepia_hud_id = citadel.sepia_hud_id
		or player:hud_add({
			hud_elem_type = "image",
...
minetest.register_on_joinplayer(function(player)
	...
	for _, stone_name in pairs({ "foward", "backward", "unlock", "break" }) do
		if inv:contains_item("main", "citadel_core:" .. stone_name .. "_stone_active") then
			for i = 1, inv:get_size("main") do
				if inv:get_stack("main", i):get_name() == "citadel_core:" .. stone_name .. "_stone_active" then
					inv:set_stack("main", i, "citadel_core:" .. stone_name .. "_stone")
				end
			end
		end
	end
	...
	for _, artifact_name in pairs({ "pendant", "coin", "totem", "scepter", "sigil", "amulet", "tablet" }) do
		if inv:contains_item("main", "citadel_core:" .. artifact_name) then
			for i = 1, inv:get_size("main") do
				if inv:get_stack("main", i):get_name() == "citadel_core:" .. artifact_name then
					inv:set_stack("main", i, "")
					player:get_meta():set_string("citadel_core:" .. artifact_name .. "_node", "obtained")
				end
			end
		end
	end
...
minetest.register_on_newplayer(function(player)
	...
	data:set_string(
		"plant_data",
		minetest.serialize({
			{ { x = 37, y = 7, z = 19 }, "vine", 3 },
			{ { x = 35, y = 2, z = 32 }, "bamboo", 2 },
			{ { x = 3, y = 2, z = 19 }, "tree", 1 },
			{ { x = 43, y = 1, z = 2 }, "big_tree", 3 },
		})
	)
